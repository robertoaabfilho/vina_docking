...
"""
Vina Batch Docking — Multi-Protein x Multi-Ligand Automation
==============================================================
Runs AutoDock Vina for every ligand in a folder against every protein
...
# ============================================================
#  Resume support (skip protein/ligand pairs already in the CSV)
# ============================================================

def load_processed_pairs(csv_path):
    """
...
# ============================================================
#  Single docking job (one protein x one ligand)
# ============================================================
...
def run_docking(
    ...
    job_key = "{}::{}".format(protein_name, ligand_name)
    # ':' is illegal in Windows filenames/paths, so use a separate,
    # filesystem-safe key for temp file names (job_key stays as-is for
    # on-screen/log display).
    safe_key = "{}__{}".format(protein_name, ligand_name)
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(suffix=".pdbqt", prefix="vina_{}_".format(safe_key))
    os.close(tmp_fd)
    out_file = Path(tmp_path)
    cmd = build_vina_command(
        vina_bin, protein_path, fixed_ligand, out_file, config, extra_args
    )

    def _set_status(msg):
        if status_dict is not None and status_lock is not None:
            with status_lock:
                status_dict[job_key] = msg
        if progress is not None and task_id is not None:
            progress.update(task_id, description=f"[cyan]{job_key:<40}[/cyan] {msg}")
...
def write_debug_log(debug_dir, protein_name, ligand_name, affinity, elapsed, err, vina_output,
    ...
    debug_logs/session.log — no per-job file is created, so nothing
...
def run_batch(
    ...
    # ── Resume support: skip (protein, ligand) pairs already recorded ─
    ensure_csv_header(csv_path)
    already_processed = load_processed_pairs(csv_path)
    if already_processed:
        remaining = [
            (prot, lig) for (prot, lig) in jobs
            if (prot["name"], lig.stem) not in already_processed
        ]
        skipped = len(jobs) - len(remaining)
        if skipped:
            log.info(
                "Retomando execução: %d par(es) proteína/ligante já presentes em %s serão ignorados.",
                skipped, csv_path.name,
            )
        jobs = remaining
        if not jobs:
            log.info("Todos os pares proteína/ligante já foram processados em %s. Nada a fazer.", csv_path.name)
            return
    ...
    workers = min(workers, 10, len(jobs))
    ...
    print(f"  Proteins : {len(proteins)} ({', '.join(p['name'] for p in proteins)})")
    print(f"  Ligands  : {len(ligands)} files")
    print(f"  Jobs     : {len(jobs)} (protein x ligand)")
    ...
    def _submit_all(executor, progress=None, tasks=None):
        return {
            executor.submit(
                run_docking,
                lig,
                ...
                extra_args,
                progress,
                tasks[(prot["name"], lig.stem)] if tasks else None,
                status_dict,
                status_lock,
                debug,
            ): (prot, lig)
            for (prot, lig) in jobs
        }
    ...
    if RICH_AVAILABLE:
        ...
        overall = progress.add_task(
            f"[bold white]Overall ({len(jobs)} jobs)", total=len(jobs)
        )
        tasks = {}
        for (prot, lig) in jobs:
            job_key = "{}::{}".format(prot["name"], lig.stem)
            tid = progress.add_task(
                f"[dim]{job_key:<40}[/dim] queued",
                total=1,
                visible=True,
            )
            tasks[(prot["name"], lig.stem)] = tid
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parallel AutoDock Vina batch docking — multiple proteins x multiple ligands",
...
Examples:
  ...
  python vina_docking.py --proteins ./proteins --ligands ./ligands FDA_extra.pdbqt

  # Text file with one ligand path per line:
  python vina_docking.py --proteins ./proteins --ligands my_list.txt

  # Extra Vina args (after --):
  python vina_docking.py --proteins ./proteins --ligands ./ligands -- --exhaustiveness 16
        """
    )
    ...
    parser.add_argument("--debug", action="store_true",
                        help="Print full Vina output for failed jobs")
